...
def get_all_products():
    sql = """SELECT id, creator_id, name, price, description,
...
def add_product(creator_id, name, price, description):
    sql = """INSERT INTO products (creator_id, name, price,
             description, active, created_at, updated_at)
             VALUES (:creator_id, :name, :price, :description, true, NOW(), NOW())"""
    try:
        db.session.execute(sql, {"creator_id":creator_id, "name":name,
...
def edit_product(product_id, new_name, new_price, new_description, active):
    sql = """UPDATE products
             SET name=:new_name, price=:new_price, description=:new_description,
             active=:is_active, updated_at=NOW()
             WHERE id=:product_id"""
    try:
        db.session.execute(sql, {"new_name":new_name, "new_price":new_price,
            "new_description":new_description, "is_active":active, "product_id":product_id})
        db.session.commit()
        if not active:
